...
const swapParams = {
  ...
  dst: ADDRESS[CHAINNAME].WETH,  // Token address of WETH
...
async function approve() {
  const signer = SIGNER;
  const prizetoken = new ethers.Contract(PRIZETOKEN, ABI.ERC20, SIGNER);

  console.log("Approving...");
  try {
    const approveTx = await prizetoken.approve(
...
async function swap() {
  console.log(await BuildTxForSwap(swapParams));
}
//swap()
//approve()
...
/*const swapParams = {
    ...
    from: ADDRESS[CHAINNAME].PRIZESANTA,
    //from: CONFIG.WALLET,
