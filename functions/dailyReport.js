...
const decimals = ADDRESS[CHAINNAME].PRIZETOKEN.DECIMALS;
...
const drawAwardedSignature =
  "0x60785c409db91938793d3f74013b06843f82ea0588265495b262b016fe5323ae";
...
async function fetchDrawAwardedLogs() {
  try {
    ...
    const logs = await PROVIDERS[CHAINNAME].getLogs(drawAwardedFilter);
...
async function findThreeMostRecentDrawsAwarded() {
  ...
  if (events.length < 3) {
    throw new Error(
      "Not enough draws found. Need at least three draws to proceed."
    );
  }
...
async function findEventsSinceLastDraw(
  drawBlockNumberStart,
  drawBlockNumberEnd
) {
  const contributeFilter = prizePoolContract.filters.ContributePrizeTokens(
    null,
    null,
    null
  );
  const contributeEvents = await prizePoolContract.queryFilter(
    contributeFilter,
    drawBlockNumberStart,
    drawBlockNumberEnd
  );

  const claimedFilter = prizePoolContract.filters.ClaimedPrize(
    null,
    null,
    null,
    null,
  ...
  const claimedEvents = await prizePoolContract.queryFilter(
    claimedFilter,
    drawBlockNumberStart,
    "latest"
  );
...
async function DailyReport() {
  const { thirdLastDraw, mostRecentDraw } =
    await findThreeMostRecentDrawsAwarded();

  const drawBlockNumberStart = thirdLastDraw.blockNumber;
  const drawBlockNumberEnd = mostRecentDraw.blockNumber;

  let { contributeEvents, claimedEvents } = await findEventsSinceLastDraw(
    drawBlockNumberStart,
    drawBlockNumberEnd
  );

  // Decode the mostRecentDrawId from the topics and subtract 1 to get draw with finished claims
  const mostRecentDrawIdDecoded = ethers.BigNumber.from(
    mostRecentDraw.topics[1]
  )
    .sub(1)
    .toString();
  ...
  contributeEvents = contributeEvents.filter(
    (event) => event.args.drawId.toString() === mostRecentDrawIdDecoded
  );
  claimedEvents = claimedEvents.filter(
    (event) => event.args.drawId.toString() === mostRecentDrawIdDecoded
  );
  ...
  const { totalPayout, totalFees, uniqueTiers, uniqueWinners } =
    processClaimedPrizes(claimedEvents);
  ...
  console.log(
    `Number of Contribute Events: ${dailyData.numberOfContributeEvents}`
  );
...
async function sendDailyReportToChannel(dailyData) {
  ...
  const embed = new MessageEmbed()
    ...
    .addFields(
      {
        name: "Prizes",
        value: dailyData.numberOfPrizeClaims.toString(),
        inline: true,
      },
      { name: "\u200B", value: "\u200B", inline: true }, // Dummy field for spacing if needed
      {
        name: "Won",
        value: `${formatPrize(dailyData.totalPayout)} ${prizeTokenSymbol}`,
        inline: true,
      },
      {
        name: "Unique Winners",
        value: dailyData.uniqueWinners.toString(),
        inline: true,
      }, // Change to inline: true
      { name: "\u200B", value: "\u200B", inline: true }, // Dummy field for spacing if needed
      { name: "Tiers", value: dailyData.uniqueTiers.join(", "), inline: true },
      {
        name: "Contributions",
        value: dailyData.numberOfContributeEvents.toString(),
        inline: true,
      }, // Change to inline: true
      { name: "\u200B", value: "\u200B", inline: true }, // Dummy field for spacing if needed
      {
        name: "Generated",
        value: `${formatPrize(
          dailyData.totalContributeAmount
        )} ${prizeTokenSymbol}`,
        inline: true,
      }
    );
