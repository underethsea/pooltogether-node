      ...
      for (let index of win.i) {
        ...
        if (BATCH_SIZE === 0) {
          ...
          try {
            ...
            if (result) {
              console.log(
                `ok: Individual call for winner ${win.p} tier ${win.t} index ${index}`
              );
            } else {
              console.log(
                `FALSE: Individual call for winner ${win.p} tier ${win.t} index ${index}`
              );
              falseCount++;
            }
          } catch (e) {
          ...
          if (
            batchCalls.length === BATCH_SIZE ||
            (win === wins[wins.length - 1] && index === win.i[win.i.length - 1])
          ) {
            // Execute batch calls
            ...
            results.forEach((result, resultIndex) => {
              if (result) {
                ...
                console.log(
                  `FALSE: Batch ${Math.ceil(
                    (falseCount + resultIndex + 1) / BATCH_SIZE
                  )}, Call ${resultIndex + 1}`
                );
                falseCount++;
              }
